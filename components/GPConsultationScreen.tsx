...
type ConsultationsScreenParams = {
  fullname: string;
  job_title: string;
...
const ConsultationsScreenCard = ({
  ...
  const navigation =
    useNavigation<
      NavigationProp<{ ConsultationsScreen: ConsultationsScreenParams }>
    >();
    ...
    <View style={tailwind("bg-white rounded-lg shadow-lg p-4 mb-4")}>
      ...
      <View style={tailwind("mt-6")}>
        ...
        <View style={tailwind("flex-row justify-between items-center mt-4")}>
          <Text style={tailwind("text-lg font-bold")}>From R600.00</Text>
          <TouchableOpacity
            style={tailwind("bg-green-500 rounded-full px-4 py-2")}
            onPress={() => {
              console.log(
                "Button Pressed: Setting loading to true and showing WebView"
              );
              setLoading(true);
      ...
      >
        ...
        >
          <View style={tailwind("relative w-full h-full")}>
            ...
            <WebView
              ref={webViewRef}
              source={{ uri: "https://test.mapw3.co.za/consult/" }}
...
export default ConsultationsScreenCard;
